    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for p in self.legalPositions:
                ...
                if trueDistance > 0:
                    allPossible[p] = initBelief[p] * emissionModel[trueDistance] #/float(len(self.legalPositions))
                else:
                    allPossible[p] = 0.0
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        # for newPos, prob in newPosDist.items():
            # print newPos, prob
        for p in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))   # P(new position | old position) * P(init belief[old pos]) for coming from all different old position to a given new position
            for newPos, prob in newPosDist.items():
                allPossible[newPos] = allPossible[newPos] + initBelief[p] * prob
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        # partList = []
        # part=self.numParticles
        # while part>=0:
        #     partList.append(self.legalPositions)
        #     part -= len(self.legalPositions)
        part = self.numParticles
        leglPos = self.legalPositions
        multpl = int(round(float(part)/float(len(leglPos))))
        partList = leglPos * multpl
        self.particles = partList[:part]
    ...
    def observe(self, observation, gameState):
        ...
        pacmanPosition = gameState.getPacmanPosition()
        "*** YOUR CODE HERE ***"
        # emission model to number of samples
        numEmissionModel=util.Counter()
        for dist in emissionModel:
            numEmissionModel[dist] = int(round(emissionModel[dist] * self.numParticles))
        ...
        if noisyDistance == None:
            ...
            initBelief = util.Counter()
            for p in self.particles:
                if p != None:
                    initBelief[p] = initBelief[p] + 1

            for p in self.legalPositions:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                if trueDistance > 0:
                    allPossible[p] = initBelief[p] * numEmissionModel[trueDistance]  # /float(len(self.legalPositions))
                else:
                    allPossible[p] = 0.0

        # if all particles receive 0 wt:
        if allPossible.totalCount() == 0:
            self.initializeUniformly(gameState)
        else:
            allPossible.normalize()
            newWtgdPart = []
            for coord in range(self.numParticles):
                newWtgdPart.append(util.sample(allPossible))
            self.particles = newWtgdPart
    ...
    def elapseTime(self, gameState):
        ...
        #     if p != None:
        #         initBelief[p] = initBelief[p] + 1
        # initBelief.normalize()
        #
        # newPosDist = util.Counter()
        # newPosPart = util.Counter()
        # allPossible = util.Counter()
        # # for newPos, prob in newPosDist.items():
        # # print newPos, prob
        # for p in self.legalPositions:
        #     newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState,
        #                                                                     p))  # P(new position | old position) * P(init belief[old pos]) for coming from all different old position to a given new position
        #     # for posdist in newPosDist:
        #     #     newPosPart[posdist] = int(round(newPosDist[posdist] * self.numParticles))
        #
        #     for newPos, prob in newPosDist.items():
        #         allPossible[newPos] = allPossible[newPos] + initBelief[p] * prob
        #
        ...
        for p in self.particles:
            ...
            newWtgdPart.append(util.sample(newPosDist))
        self.particles = newWtgdPart
        # util.raiseNotDefined()
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        part = self.numParticles
        leglPos = self.legalPositions
        partListEachGhost = [x for x in itertools.product(leglPos, repeat = self.numGhosts)]

        multpl = int(math.ceil(float(part) / float(len(partListEachGhost))))
        partList = partListEachGhost * multpl
        random.shuffle(partList)
        partList = partList[:part]
        self.particles = partList
        # chk = list(itertools.product(partList))
        # self.particles = chk
        # for i in range(self.numGhosts):
        #     partListEachGhost.append(tuple(partList))
        # self.particles = partListEachGhost
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        # emission model to number of samples
        numEmissionModel = [util.Counter() for i in range(self.numGhosts)]
        for g in range(self.numGhosts):
            for dist in emissionModels[g]:
                numEmissionModel[g][dist] = int(round(emissionModels[g][dist] * self.numParticles))
        ...
        for p in self.particles:
            ...
            for g in range(self.numGhosts):
                ...
                if noisyDistances[g] == None:
                    ...
                    # if trueDistance > 0:
                    transProb = transProb * emissionModels[g][trueDistance]  # /float(len(self.legalPositions))
            allPossible[p] += transProb
